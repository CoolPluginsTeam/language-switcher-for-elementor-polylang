...
if ( ! class_exists( 'LSEP_LanguageSwitcher' ) ) {
	...
	class LSEP_LanguageSwitcher {
		...
		public function lsep_activate() {
			\LSEP_Plugin_Migration::block_activation_if_successor_present( LSEP_PLUGIN_FILE );
		...
		public function lsep_redirect_to_settings() {
			if ( \LSEP_Plugin_Migration::is_successor_present() ) {
				return;
			}
		...
		public function lsep_init() {
			\LSEP_Plugin_Migration::init();

			// Successor plugin present: stop loading this plugin's features.
			if ( \LSEP_Plugin_Migration::is_successor_present() ) {
				return;
			}
